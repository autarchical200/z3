...
order.onclick = function () {
  alert("Bạn cần đăng nhập để thanh toán đơn hàng này.")
}
...
// xóa cart
var remove_cart = document.getElementsByClassName("xoa");
for (var i = 1; i < remove_cart.length; i++) {
...
function updatecart() {
  ...
  for (var i = 0; i < cart_rows.length; i++) {
    ...
    var price = parseFloat(price_item.innerText.replace(/,/g,''))// chuyển một chuổi string sang number để tính tổng tiền.
...
// Thêm vào giỏ
var add_cart = document.getElementsByClassName("them");
for (var i = 0; i < add_cart.length; i++) {
  ...
  add.addEventListener("click", function (event) {
    ...
    var img = product.getElementsByClassName("card-img-top")[0].src
...
function addItemToCart(title, price, img) {
  ...
  <div class="cart-item cart-column">
      <img class="cart-item-image rounded-0" src="${img}" width="100" height="100">
      <span class="cart-item-title text-light">${title}</span>
  </div>
  <span class="cart-price cart-column">${price}</span>
  <div class="cart-quantity cart-column">
      <input class="cart-quantity-input bg-light rounded-0" type="number" value="1">
      <button class="btn text-light btn-danger rounded-0"  type="button"><i class="bi bi-trash-fill"></i> Xóa</button>
  </div>`
